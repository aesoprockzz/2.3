	...
	public class TownCrier : Mobile, ITownCrierEntryList
	{
		...
		public void ForceBeginAutoShout()
		{
			if ( m_AutoShoutTimer == null )
				m_AutoShoutTimer = Timer.DelayCall( TimeSpan.FromSeconds( 5.0 ), TimeSpan.FromSeconds( 15.0 ), new TimerCallback( AutoShout_Callback ) );
		}
		...
		private void AutoShout_Callback()
		{
			...
			{
				m_NewsTimer = Timer.DelayCall( TimeSpan.FromSeconds( 0.0 ), TimeSpan.FromSeconds( 0.0 ), new TimerStateCallback( ShoutNews_Callback ), new object[]{ tce, 0 } );
		...
		{
			if ( m_NewsTimer == null && InRange( e.Mobile, 12 ) ) // *news*
			{
