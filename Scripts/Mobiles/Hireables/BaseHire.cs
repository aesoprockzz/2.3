...
namespace Server.Mobiles
{
	public class BaseHire : BaseCreature
	{
		...
		public bool Think()
		{
			...
			if ( targ != null )
			{
				Mobile toHelp = Find( GetOwner() );

				if ( toHelp != null )
				{
					targ.Invoke( this, toHelp );
				}
				else
				{
					targ.Cancel( this, TargetCancelType.Canceled );
				}
			}*/
			return true;
		}
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			m_IsHealer = reader.ReadBool();
			m_HoldGold = reader.ReadInt();

			m_PayTimer = new PayTimer( this );
			m_PayTimer.Start();
			
			m_HealTimer = new HealTimer( this );
			m_HealTimer.Start();
		}
		...
		#region [ Class HealTimer ] 
		private class HealTimer : Timer 
		{ 
			private BaseHire m_Hire;
	  
			public HealTimer( BaseHire vend ) : base( TimeSpan.FromSeconds( 5.0 ), TimeSpan.FromSeconds( 5.0 ) ) 
			{ 
				m_Hire = vend;
			...
			{ 
				//Mobile owner = m_Hire.GetOwner();
				m_Hire.Say( 503235 );
				/*if((owner.HitsMax - owner.Hits) < (owner.HitsMax - 25))
				{
					Mobile toHelp = owner;

					if ( toHelp != null )
					{
						if ( !(new GreaterHealSpell( m_Hire, null )).Cast() )
								new HealSpell( m_Hire, null ).Cast();
					}
